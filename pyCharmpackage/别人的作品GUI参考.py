# -*- coding = utf-8 -*-
# by: https://blog.csdn.net/qq_28969139/article/details/81008389
...
class Chess(object):
    ...
    def is_win(self, x, y, tag):
        ...
        four_direction = [[self.matrix[i][y] for i in range(self.row)], [self.matrix[x][j] for j in range(self.column)],
                          direction(x, y, 1, 1, self.row, self.column, self.matrix),
                          direction(x, y, 1, -1, self.row, self.column, self.matrix)]
        # 获取水平和竖直方向的列表
        # 获取斜方向的列表
